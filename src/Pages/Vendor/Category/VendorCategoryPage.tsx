...
const VendorCategoryPage = () => {
  ...
  // Update only the filtering part of your VendorCategoryPage component

  const filteredCategories = categories
    .filter(category => {
      // For Active tab (index 0), show approved categories
      // For Pending tab (index 1), show non-approved categories
      if (activeTab === 0) {
        return category.approved === true;
      } else {
  ...
  // Update the count calculations
  const activeCount = categories.filter(category => category.approved === true).length;
  const pendingCount = categories.filter(category => category.approved === false).length;
          ...
          <div className="border-b border-gray-200">
            <nav className="-mb-px flex space-x-8">
              {['Active', 'Pending'].map((tab, index) => (
                ...
                >
                  {tab} ({index === 0 ? activeCount : pendingCount})
                </button>
              ))}
            </nav>
          </div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                {searchTerm
                  ...
                  : activeTab === 0
                    ...
                    : "No approved categories at the moment."}
              </p>
            </div>
          )}
        </div>
